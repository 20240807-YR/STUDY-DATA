...
# 설정
# =========================
# 라네즈 브랜드 URL (정확한 brandSn=31)
BRAND_URL = "https://www.amoremall.com/kr/ko/display/brand/detail/all?brandSn=31"

BASE_DIR = Path(__file__).resolve().parent.parent
OUTPUT_CSV = BASE_DIR / "data_csv" / "아모레" / "라네즈_세부.csv"
...
async def wait_for_manual_popup_close(page):
    ...
    while True:
        still_open = await page.locator(popup_selector).count()
        if still_open == 0:
            break
        await page.wait_for_timeout(300)
    print("👉 팝업 닫힘 확인됨. 다음 단계 진행.")
...
async def extract_ingredients(page):
    try:
        ...
        xpaths = [
            ...
            "xpath=//*[contains(., '화장품법')]/parent::*[1]/following-sibling::*[1]",
...
async def main():
    ...
    print("\n====================================")
    print("🎉 라네즈 CSV 생성 완료 (brandSn=31)!")
